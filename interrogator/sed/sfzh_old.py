...
def constant(log10ages, log10Zs, p):
    ...
    # 'log10M*' - total stellar mass
    # 'log10_duration' - total duration of star formation
    # 'log10Z' - metallicity
...
def constant_new(log10ages, log10Zs, p):

    # 'log10M*' - total stellar mass
    # 'log10_duration' - total duration of star formation
    # 'log10Z' - metallicity

    total_duration = 10**(p['log10_duration'])
    ...
    for ia, log10age in enumerate(log10ages[:-1]):
        ...
        sf = integrate.quad(const, min_age, max_age, args=(total_duration))[0]
        SFZH[ia,iZ] = sf
        min_age = max_age
...
def quenched(log10ages, log10Zs, p):
    ...
    # 'log10M*' - total stellar mass
    # 'log10_SF_duration' - total duration of star formation, before being quenched
    # 'log10_quenched_duration' - total duration of quenching
    # 'log10Z' - metallicity

    total_duration = 10**p['log10_SF_duration'] + 10**p['log10_quenched_duration']
    quenched_duration = 10**p['log10_quenched_duration']
    ...
    sfh[int(total_duration):] = 0.0

    min_age = 0
    for ia, log10age in enumerate(log10ages[:-1]):
        max_age = int(10**np.mean([log10ages[ia+1],log10age]))
        SFZH[ia,iZ] = np.sum(sfh[min_age,max_age])
        min_age = max_age
